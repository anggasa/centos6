...
def speedtest():
    ...
    if args.server:
        ...
        if not args.simple:
            print_('Ozikhacker Akan Memilih Server')
        best = getBestServer(servers)

    if not args.simple:
        print_('Server Ini Di Produksi Oleh %(sponsor)s (%(name)s) [%(d)0.2f km]: '
               '%(latency)s ms' % best)
    else:
    ...
    if not args.simple:
        print_('Test Kecepatan Download', end='')
    ...
    for size in sizesizes:
        for i in range(0, 25):
            sizes.append(size)
    if not args.simple:
        print_('Test Kecepatan Upload', end='')
